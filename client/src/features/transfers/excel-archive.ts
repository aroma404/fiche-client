...
function detailedClientSheet(bundle: ClientDraft, index: number) {
  const { client } = bundle;
  const financeEntries = bundle.financeEntries.length ? bundle.financeEntries : [...bundle.payments.map(item => ({ entryDate: item.paymentDate, category: "Paiement", direction: "Entrée", label: item.label, reference: item.reference, amount: item.amount })), ...bundle.cashEntries.map(item => ({ entryDate: item.entryDate, category: "Caisse", direction: item.direction, label: item.label, reference: "", amount: item.amount }))];
  ...
  appendSection(rows, "RÉFÉRENCES FISCALES ET JURIDIQUES", [["NIF", client.nif], ["N° RC", client.rc], ["BP", client.bp], ["Article d’imposition", client.taxArticle], ["NIN", client.nin], ["Régime fiscal", client.regime], ["Centre d’impôt", client.taxCenter]]);
  appendSection(rows, "PAIEMENTS ET OBSERVATIONS (DA)", [["Solde initial", client.initialBalance], ["Paiements liés", paymentTotal(bundle)], ["Solde estimé", client.initialBalance - paymentTotal(bundle)], ["Observations", client.observations || "Aucune observation financière renseignée."]]);
  ...
  const sectionNames = new Set(["IDENTITÉ ET ACTIVITÉ", "RÉFÉRENCES FISCALES ET JURIDIQUES", "PAIEMENTS ET OBSERVATIONS (DA)", "DOCUMENTS", "CONFORMITÉ", "OPÉRATIONS LIÉES AU CLIENT"]);
...
async function createStyledExcelArchive(payload: ExportPayload) {
  ...
  const archiveSheetNames = ["Clients", "Documents", "Conformité", "Opérations"];
  ...
  const operations: ArchiveRow[] = clients.flatMap((bundle, index) => (bundle.financeEntries.length ? bundle.financeEntries : [...bundle.payments.map(item => ({ entryDate: item.paymentDate, category: "Paiement", direction: "Entrée", label: item.label, reference: item.reference, amount: item.amount })), ...bundle.cashEntries.map(item => ({ entryDate: item.entryDate, category: "Caisse", direction: item.direction, label: item.label, reference: "", amount: item.amount }))]).map(item => ({ "Clé dossier": index + 1, Date: item.entryDate, Nature: item.category, Libellé: item.label, Sens: item.direction, Référence: item.reference, "Montant (DA)": item.amount })));
  const tables: [string, string, string, ArchiveRow[], string[], number[]][] = [["Clients", "DOSSIERS CLIENTS", "Identité et références fiscales", clientRows, ["Clé dossier", "Nom / raison sociale", "Activité", "Forme juridique", "Type de client", "Statut", "Commune", "Contact", "NIF", "N° RC", "BP", "Article d’imposition", "NIN", "Régime fiscal", "Centre d’impôt", "Solde initial (DA)", "Observations financières"], [12, 30, 24, 20, 18, 14, 18, 20, 16, 14, 12, 20, 16, 22, 18, 18, 36]], ["Documents", "DOCUMENTS", "Pièces et statut documentaire", docs, ["Clé dossier", "Document", "Catégorie", "Statut", "Observation"], [12, 28, 18, 16, 42]], ["Conformité", "CONFORMITÉ", "Obligations sociales et administratives", compliance, ["Clé dossier", "Obligation", "Statut", "Note"], [12, 32, 18, 42]], ["Opérations", "OPÉRATIONS CLIENT", "Paiements et caisse liés au client", operations, ["Clé dossier", "Date", "Nature", "Libellé", "Sens", "Référence", "Montant (DA)"], [12, 14, 16, 30, 16, 22, 18]]];
