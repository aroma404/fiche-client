...
export default function DocumentsPlugin() {
  ...
  return <div>
    ...
    <SectionCard title="Checklist documentaire" hint="Une pièce peut être reçue, à vérifier, non requise ou à demander."><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead><tr className="border-b-2 border-[#cfdad7] text-[10px] font-extrabold uppercase tracking-[0.1em] text-[#647987]"><th className="px-3 py-2.5">Document</th><th className="px-3 py-2.5">Catégorie</th><th className="px-3 py-2.5">Statut</th><th className="px-3 py-2.5">Observation</th></tr></thead><tbody>{state.documents.map((document) => <tr key={document.id} className="border-b border-[#e7edeb] last:border-0"><td className="px-3 py-2.5 text-sm font-bold text-[#102a43]">{document.label}</td><td className="px-3 py-2.5 text-xs text-[#627785]">{document.category}</td><td className="px-3 py-2.5"><AppSelect ariaLabel={`Statut ${document.label}`} value={document.status} onValueChange={value => dispatch({ type: "DOCUMENT", id: document.id, patch: { status: value as Status } })} options={statuses.map(status => ({ value: status, label: status }))} /></td><td className="px-3 py-2.5"><input aria-label={`Observation ${document.label}`} value={document.note} onChange={(event) => dispatch({ type: "DOCUMENT", id: document.id, patch: { note: event.target.value } })} placeholder="Précision courte" className="h-8 w-full min-w-[180px] rounded-md border border-[#d7e0df] bg-[#fbfcfa] px-3 text-xs outline-none focus:ring-2 focus:ring-[#0f766e]" /></td></tr>)}</tbody></table></div></SectionCard>
