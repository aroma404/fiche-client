...
const privacyImage = "/manus-storage/confidentialite-session_e86267fc.png";
...
export default function PrivacyPlugin() {
  ...
  return <div><PageHeader eyebrow="Confidentialité par défaut" title="Une session locale, sans historique" description="La fiche existe seulement dans l’état de cette page. Elle n’est pas synchronisée, envoyée ou enregistrée automatiquement." />
    <div className="grid gap-6 xl:grid-cols-[1fr_1fr]"><section className="relative min-h-[390px] overflow-hidden rounded-[1.25rem] border border-[#d7e0df] bg-[#ebefeb]"><img src={privacyImage} alt="Dossier anonyme protégé" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#102a43] via-[#102a43]/80 to-transparent p-7 pt-24 text-white"><p className="text-[10px] font-extrabold uppercase tracking-[0.13em] text-[#9bd4c7]">Principe de session</p><h2 className="mt-2 font-serif text-3xl">Vos saisies restent ici, puis disparaissent.</h2><p className="mt-3 max-w-md text-sm leading-6 text-[#d8e4e8]">Le rechargement, la fermeture de l’onglet ou l’effacement manuel réinitialisent les données temporaires.</p></div></section>
