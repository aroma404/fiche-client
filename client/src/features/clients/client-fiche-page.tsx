  ...
  return <WorkspaceLayout>
    <div className="mb-7">
      ...
      <div className="mt-4 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div><p className="text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#0f766e]">Dossier client actif</p><h1 className="mt-2 font-serif text-4xl text-[#102a43]">Fiche du client</h1></div>
        <div className="flex flex-wrap gap-2"><Link href={`/transferts?clientId=${clientId}`} className="inline-flex h-10 items-center justify-center rounded-lg border border-[#cfdad7] bg-white px-4 text-sm font-bold text-[#102a43]">Exporter ce client</Link><button disabled={save.isPending} onClick={() => save.mutate({ clientId, data: draft })} className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#0f766e] px-4 text-sm font-bold text-white disabled:opacity-60"><Save size={16} />{save.isPending ? "Enregistrement…" : "Enregistrer"}</button></div>
      </div>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <FormCard title="1. Informations générales">
        <TextGrid draft={draft} onChange={setClient} fields={[["Nom / raison sociale", "fullName"], ["Commune", "commune"]]} />
      ...
      <FormCard title="2. Références fiscales et juridiques">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2"><SelectField label="Régime fiscal" value={draft.client.regime} onChange={value => setClient("regime", value)} choices={regimeChoices} /><SelectField label="Centre d’impôt" value={draft.client.taxCenter} onChange={value => setClient("taxCenter", value)} choices={["CDI", "CPI"]} /></div>
        <div className="mt-4 border-t border-[#e6eeeb] pt-4"><p className="ui-label">Affiliations sociales</p><div className="mt-3 grid gap-4 sm:grid-cols-2"><SelectField label="Affilié à la CNAS" value={draft.client.cnasAffiliated ? "Oui" : "Non"} onChange={value => setDraft({ ...draft, client: { ...draft.client, cnasAffiliated: value === "Oui" } })} choices={yesNoChoices} /><SelectField label="Affilié au CASNOS" value={draft.client.casnosAffiliated ? "Oui" : "Non"} onChange={value => setDraft({ ...draft, client: { ...draft.client, casnosAffiliated: value === "Oui" } })} choices={yesNoChoices} /></div></div>
