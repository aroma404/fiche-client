...
export function AppSelect({ value, onValueChange, options, placeholder, id, ariaLabel, disabled }: { value: string; onValueChange: (value: string) => void; options: readonly AppSelectOption[]; placeholder?: string; id?: string; ariaLabel?: string; disabled?: boolean }) {
  return <Select value={value || emptyValue} onValueChange={nextValue => onValueChange(nextValue === emptyValue ? "" : nextValue)} disabled={disabled}><SelectTrigger id={id} aria-label={ariaLabel} className="h-11 min-h-11 w-full rounded-none border-[#b9d4ca] bg-[#fffefa] px-3 text-sm font-bold text-[#102a43] shadow-none transition focus:border-[#0f766e] focus:ring-2 focus:ring-[#0f766e]/15"><SelectValue placeholder={placeholder ?? "Sélectionner une option"} /></SelectTrigger><SelectContent className="rounded-none border-[#c9ddd6] bg-[#fffefa] text-[#102a43] shadow-[0_14px_32px_rgba(16,42,67,0.14)]">{options.map(option => <SelectItem key={option.value || emptyValue} value={option.value || emptyValue} disabled={option.disabled} className="min-h-11 cursor-pointer rounded-none py-2.5 text-sm font-semibold focus:bg-[#eaf6f1] focus:text-[#0f766e]">{option.label}</SelectItem>)}</SelectContent></Select>;
}
