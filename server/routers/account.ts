...
import { z } from "zod";
import { accounts, accountSessions, clientCashEntries, clientCompliance, clientDocuments, clientPayments, clients, clientWorkCases, exportAudit, passwordVaultEntries, programClientStatuses } from "../../drizzle/schema";
...
export const accountRouter = router({
  ...
  deleteAccount: publicProcedure.input(z.object({ currentPassword: z.string().min(1), confirmation: z.literal("SUPPRIMER") })).mutation(async ({ ctx, input }) => {
    ...
    await db.transaction(async tx => {
      ...
      await Promise.all([tx.delete(exportAudit).where(eq(exportAudit.accountId, account.id)), tx.delete(passwordVaultEntries).where(eq(passwordVaultEntries.accountId, account.id)), tx.delete(programClientStatuses).where(eq(programClientStatuses.accountId, account.id)), tx.delete(accountSessions).where(eq(accountSessions.accountId, account.id))]);
