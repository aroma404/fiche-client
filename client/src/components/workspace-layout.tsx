/** Atelier fiscal moderne — coque privée du bureau comptable, limitée au compte connecté. */

import { BrandSymbol } from "@/components/local-visuals";
import { useAuth } from "@/_core/hooks/useAuth";
import { trpc } from "@/lib/trpc";
import { Download, LayoutDashboard, LogOut, Plus, Settings, Users } from "lucide-react";
...
function sectionTitle(location: string) {
  if (location.startsWith("/dashboard")) return "Tableau de bord";
  if (location.startsWith("/clients")) return "Dossiers clients";
  if (location.startsWith("/transferts")) return "Importation et exportation";
  return "Mon compte";
}
...
export function WorkspaceLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f6f5f0] lg:flex">
    <aside className="sidebar-shell hidden h-screen w-[292px] shrink-0 flex-col border-r border-[#cfdad7] bg-[#edf1ee] px-5 py-6 lg:flex">
      ...
      <div className="mt-7 min-h-0 flex-1"><div className="mb-3 flex items-center justify-between"><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#58727a]">Clients actifs</p><button onClick={() => setLocation("/clients?nouveau=1")} aria-label="Nouveau client" className="rounded-md p-1 text-[#0f766e] hover:bg-white"><Plus size={16} /></button></div><div className="max-h-[45vh] space-y-1 overflow-y-auto pr-1">{clientsQuery.data?.map(client => <Link key={client.id} href={`/clients/${client.id}/fiche`} className={`block rounded-lg border px-3 py-2 text-sm transition ${location.includes(`/clients/${client.id}`) ? "border-[#0f766e] bg-white text-[#102a43]" : "border-transparent text-[#526775] hover:border-[#d7e0df] hover:bg-white/70"}`}><p className="truncate font-bold">{client.fullName}</p><p className="mt-0.5 text-[11px] text-[#76909a]">{client.status} · {client.commune || "Sans commune"}</p></Link>)}{clientsQuery.data?.length === 0 ? <p className="rounded-lg border border-dashed border-[#cfdad7] p-3 text-xs leading-5 text-[#627785]">Aucun client. Créez votre premier dossier.</p> : null}</div></div>
      <div className="border-t border-[#cfdad7] pt-4"><p className="truncate text-sm font-bold text-[#102a43]">{user.fullName}</p><p className="mt-1 truncate text-xs text-[#627785]">{user.email}</p><button onClick={() => void logout().then(() => setLocation("/"))} className="mt-3 flex w-full items-center gap-2 rounded-lg border border-[#e1d4d1] bg-white px-3 py-2 text-xs font-bold text-[#a1433d] hover:bg-[#fff6f4]"><LogOut size={15} /> Se déconnecter</button></div>
    </aside>
