...
const documentInput = z.object({ id: z.number().int().positive().optional(), label: z.string().min(1).max(120), category: z.string().max(100).default("Fiscal"), status: z.string().min(2).max(100).default("À demander"), note: z.string().max(500).default("") });
...
const documentMutationInput = z.object({ label: z.string().trim().min(1).max(120), category: z.string().trim().min(2).max(100), status: z.string().trim().min(2).max(100), note: z.string().trim().max(500).default("") });
