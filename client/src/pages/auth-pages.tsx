...
function UsageConvention({ compact = false, acknowledged = false, onAcknowledgedChange }: { compact?: boolean; acknowledged?: boolean; onAcknowledgedChange?: (value: boolean) => void }) {
  ...
  return <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm font-semibold text-[#526775]"><label className="flex cursor-pointer items-center gap-2"><input checked={acknowledged} onChange={event => onAcknowledgedChange?.(event.target.checked)} type="checkbox" className="accent-[#0f766e]" />Convention d’utilisation</label><Link href="/convention-utilisation" className="text-sm font-bold text-[#0f766e] underline decoration-[#9bcfc0] underline-offset-2 hover:text-[#0b625d]">Lire la convention</Link></div>;
}
...
function AuthCard({ mode }: { mode: "login" | "register" }) {
  ...
  const [conventionRead, setConventionRead] = useState(false);
