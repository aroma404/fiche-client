...
export function WorkspaceLayout({ children }: { children: ReactNode }) {
  ...
  const statusesQuery = trpc.programSettings.clientStatuses.list.useQuery(undefined, { enabled: Boolean(user), staleTime: 30_000 });
  const [search, setSearch] = useState("");
  const allClients = (clientsQuery.data ?? []) as WorkspaceClient[];
  const operationalStatusLabels = (statusesQuery.data?.filter(status => status.isOperational).map(status => status.label) ?? ["Actif"]);
  const clients = allClients.filter(client => isOperationalClient(client, operationalStatusLabels));
  ...
  return <WorkspaceClientsContext.Provider value={{ clients, allClients, operationalStatusLabels, isLoading: clientsQuery.isLoading }}><div className="min-h-screen bg-[#f6f5f0] lg:flex">
    <aside className="sidebar-shell hidden h-screen w-[292px] shrink-0 flex-col border-r border-[#cfdad7] bg-[#edf1ee] px-5 py-6 lg:flex">
      ...
      <div className="mt-6"><p className="mb-3 text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#58727a]">Espace de travail</p><nav className="space-y-1">{workspaceFeatures.map(({ href, label, icon: Icon }) => <FastPrivateLink key={href} href={href} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-bold transition ${location.startsWith(href) ? "bg-[#dff1ea] text-[#0f766e]" : "text-[#526775] hover:bg-white/75"}`}><Icon size={17} />{label}</FastPrivateLink>)}</nav></div>
    ...
    <div className="min-w-0 flex-1"><header className="topbar-shell sticky top-0 z-20 flex min-h-[72px] items-center justify-between border-t-[3px] border-[#c99a3e] border-b border-[#d7e0df] bg-[#f6f5f0]/95 px-5 backdrop-blur lg:px-10"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-[#0f766e]">Compte sécurisé</p><p className="text-sm font-bold text-[#102a43]">{workspaceSectionTitle(location)}</p></div><div className="flex items-center gap-2 text-xs font-bold text-[#627785]"><span className="h-2 w-2 rounded-full bg-[#0f766e]" /> Session active</div></header><main className="workspace-main mx-auto max-w-[1400px] p-5 lg:p-10">{children}</main></div>
