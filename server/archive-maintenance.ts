...
import { cabinetFinanceEntries, clientCashEntries, clientCompliance, clientContacts, clientDocuments, clientPayments, clients, clientWorkCases, passwordVaultEntries, programClientOptions, programClientStatuses } from "../drizzle/schema";
...
export async function purgeExpiredArchive(now = new Date()) {
  ...
  const [contacts, options, statuses, vault] = await Promise.all([
  ...
  return { clients: clientIds.length, contacts: contacts[0]?.affectedRows ?? 0, options: options[0]?.affectedRows ?? 0, statuses: statuses[0]?.affectedRows ?? 0, vault: vault[0]?.affectedRows ?? 0 };
}
