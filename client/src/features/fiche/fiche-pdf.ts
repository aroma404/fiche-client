...
export async function downloadFichePdf(draft: ClientDraft) {
  ...
  const payments = draft.payments.reduce((sum, item) => sum + Number(item.amount || 0), 0);
  ...
  field(pdf, 12, 115, "Article d’imposition", draft.client.taxArticle, 83); field(pdf, 108, 115, "Régime", draft.client.regime);

  section(pdf, 131, "Situation financière");
  const cards = [["SOLDE INITIAL", money(draft.client.initialBalance), palette.paper], ["VERSEMENTS", money(payments), [232, 246, 240] as const], ["SOLDE FINAL", money(draft.client.initialBalance - payments), [255, 248, 229] as const]];
  cards.forEach(([label, value, color], index) => { const x = 12 + index * 62; pdf.setFillColor(...(color as readonly [number, number, number])); pdf.roundedRect(x, 138, 56, 19, 2, 2, "F"); pdf.setTextColor(...palette.muted); pdf.setFont("helvetica", "bold"); pdf.setFontSize(6.5); pdf.text(label as string, x + 4, 145); pdf.setTextColor(...palette.navy); pdf.setFontSize(10); pdf.text(value as string, x + 4, 152); });

  section(pdf, 169, "Pièces reçues");
  ...
  pdf.text(pdf.splitTextToSize(pieceText, 180), 16, 178);

  section(pdf, 207, "Observations");
  pdf.setFont("helvetica", "normal"); pdf.setFontSize(8.5); pdf.setTextColor(...palette.navy); pdf.text(pdf.splitTextToSize(draft.client.observations || "Aucune observation renseignée.", 180).slice(0, 5), 16, 216);
