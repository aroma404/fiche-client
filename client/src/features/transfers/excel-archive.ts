...
function detailedClientSheet(bundle: ClientDraft, index: number) {
  ...
  appendSection(rows, "IDENTITÉ ET ACTIVITÉ", [["Référence client", client.referenceNumber ? String(client.referenceNumber).padStart(3, "0") : "—"], ["Nom / raison sociale", client.fullName], ["Activité", client.activity], ["Domaine d’activité", client.activityKind || "Non précisé"], ["Type d’auto-entreprise", client.autoEntrepreneurActivity || "—"], ["Catégorie RC", client.rcActivityFamily || "—"], ["Code activité RC", client.rcActivityCode || "—"], ["Forme juridique", client.legalForm], ["Type de client", client.clientType], ["Statut", client.status], ["Adresse", client.commune], ["Contacts", bundle.contacts.length ? bundle.contacts.map(item => `${item.label || item.type} : ${item.value}`).join(" · ") : client.contact || "—"]]);
  appendSection(rows, "RÉFÉRENCES FISCALES, JURIDIQUES ET SOCIALES", [["NIF", client.nif], ["N° RC", client.rc], ["BP", client.bp], ["Article d’imposition", client.taxArticle], ["NIN", client.nin], ["Régime fiscal", client.regime], ["Centre d’impôt", client.taxCenter], ["Affilié à la CNAS", client.cnasAffiliated ? "Oui" : "Non"], ["Affilié au CASNOS", client.casnosAffiliated ? "Oui" : "Non"], ["Affilié au CACOBATPH", client.cacobatphAffiliated ? "Oui" : "Non"]]);
  ...
  rows.push(["FICHIERS ASSOCIÉS"], ["Nom affiché", "Catégorie", "Fichier d’origine", "Document associé", "Taille (octets)"], ...(bundle.files.length ? bundle.files.map(item => [item.displayName, item.category, item.originalName, item.documentId ?? "—", item.sizeBytes]) : [["Aucun fichier", "", "", "", ""]]), []);
  rows.push(["OPÉRATIONS LIÉES AU CLIENT"], ["Date", "Nature", "Libellé", "Sens", "Référence", "Document associé", "Montant (DA)"], ...(financeEntries.length ? financeEntries.map(item => [item.entryDate, item.category, item.label, item.direction, item.reference, item.documentId ?? "—", item.amount]) : [["", "", "Aucune opération", "", "", "", 0]]));
  const ws = XLSXStyle.utils.aoa_to_sheet(rows);
  const sectionNames = new Set(["IDENTITÉ ET ACTIVITÉ", "RÉFÉRENCES FISCALES, JURIDIQUES ET SOCIALES", "PAIEMENTS ET OBSERVATIONS (DA)", "DOCUMENTS", "FICHIERS ASSOCIÉS", "OPÉRATIONS LIÉES AU CLIENT"]);
...
async function createStyledExcelArchive(payload: ExportPayload) {
  ...
  const archiveSheetNames = ["Clients", "Contacts", "Documents", "Fichiers", "Opérations"];
  ...
  const clientRows: ArchiveRow[] = clients.map((bundle, index) => ({ "Clé dossier": index + 1, "Référence client": bundle.client.referenceNumber ? String(bundle.client.referenceNumber).padStart(3, "0") : "", "Nom / raison sociale": bundle.client.fullName, Activité: bundle.client.activity, "Domaine d’activité": bundle.client.activityKind, "Type d’auto-entreprise": bundle.client.autoEntrepreneurActivity, "Catégorie RC": bundle.client.rcActivityFamily, "Code activité RC": bundle.client.rcActivityCode, "Forme juridique": bundle.client.legalForm, "Type de client": bundle.client.clientType, Statut: bundle.client.status, Adresse: bundle.client.commune, Contact: bundle.client.contact, NIF: bundle.client.nif, "N° RC": bundle.client.rc, BP: bundle.client.bp, "Article d’imposition": bundle.client.taxArticle, NIN: bundle.client.nin, "Régime fiscal": bundle.client.regime, "Centre d’impôt": bundle.client.taxCenter, "Affilié à la CNAS": bundle.client.cnasAffiliated ? "Oui" : "Non", "Affilié au CASNOS": bundle.client.casnosAffiliated ? "Oui" : "Non", "Affilié au CACOBATPH": bundle.client.cacobatphAffiliated ? "Oui" : "Non", "Solde initial (DA)": Number(bundle.client.initialBalance ?? 0), "Observations financières": bundle.client.observations }));
  ...
  const files: ArchiveRow[] = clients.flatMap((bundle, index) => bundle.files.map(item => ({ "Clé dossier": index + 1, "Nom affiché": item.displayName, Catégorie: item.category, "Fichier d’origine": item.originalName, "Type MIME": item.mimeType, "Document associé": item.documentId ?? "", "Taille (octets)": item.sizeBytes })));
  ...
  const tables: [string, string, string, ArchiveRow[], string[], number[]][] = [["Clients", "DOSSIERS CLIENTS", "Identité, activité et références fiscales et sociales", clientRows, ["Clé dossier", "Référence client", "Nom / raison sociale", "Activité", "Domaine d’activité", "Type d’auto-entreprise", "Catégorie RC", "Code activité RC", "Forme juridique", "Type de client", "Statut", "Adresse", "Contact", "NIF", "N° RC", "BP", "Article d’imposition", "NIN", "Régime fiscal", "Centre d’impôt", "Affilié à la CNAS", "Affilié au CASNOS", "Affilié au CACOBATPH", "Solde initial (DA)", "Observations financières"], [12, 16, 30, 30, 24, 24, 22, 20, 20, 18, 14, 24, 20, 16, 14, 12, 20, 16, 22, 18, 18, 18, 20, 18, 36]], ["Contacts", "CONTACTS", "Coordonnées administratives du dossier", contacts, ["Clé dossier", "Libellé", "Type", "Coordonnée", "Principal"], [12, 24, 16, 40, 14]], ["Documents", "DOCUMENTS", "Pièces et statut documentaire", docs, ["Clé dossier", "Document", "Catégorie", "Statut", "Observation"], [12, 28, 18, 16, 42]], ["Fichiers", "FICHIERS", "Métadonnées des fichiers du stockage sécurisé", files, ["Clé dossier", "Nom affiché", "Catégorie", "Fichier d’origine", "Type MIME", "Document associé", "Taille (octets)"], [12, 30, 18, 30, 28, 18, 18]], ["Opérations", "OPÉRATIONS CLIENT", "Paiements et caisse liés au client", operations, ["Clé dossier", "Date", "Nature", "Libellé", "Sens", "Référence", "Montant (DA)"], [12, 14, 16, 30, 16, 22, 18]]];
