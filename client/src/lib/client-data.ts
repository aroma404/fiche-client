...
export type ClientDraft = {
  ...
  documents: { id?: number; label: string; category: string; status: DocumentStatus; note: string }[];
  ...
  financeEntries: { id?: number; documentId?: number | null; entryDate: string; category: "Paiement" | "Caisse"; direction: "Entrée" | "Sortie"; label: string; reference: string; amount: number; note: string }[];
...
export function normalizeBundle(raw: any): ClientDraft {
  ...
  return {
    ...
    documents: (raw.documents ?? fallback.documents).map((x: any) => ({ id: x.id, label: String(x.label ?? "").replace(/^TAP\s*\/\s*TFPC$/i, "TAPP"), category: x.category ?? "Fiscal", status: String(x.status ?? "À demander"), note: x.note ?? "" })),
    ...
    financeEntries: (raw.financeEntries ?? []).map((x: any) => ({ id: x.id, documentId: x.documentId ?? null, entryDate: x.entryDate ?? x.date ?? "", category: x.category === "Caisse" ? "Caisse" : "Paiement", direction: x.direction === "Sortie" ? "Sortie" : "Entrée", label: x.label ?? "", reference: x.reference ?? "", amount: Number(x.amount ?? 0), note: x.note ?? "" })),
